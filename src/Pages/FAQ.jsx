...
function Row(props) {
  ...
  return (
    <React.Fragment>
      <TableRow component="button" onClick={() => {open === index ? setOpen(-1) : setOpen(index)}} classes={{ root: classes.root }}>
      ...
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open === index} timeout="auto" unmountOnExit>
            <Box margin={1}>
              <Typography variant="h6" classes={{ root: classes.data }} gutterBottom component="div">
                {row.data}
              </Typography>
            </Box>
          </Collapse>
...
const rows = [
  ...
  createData("¿Cómo puedo crear y compartir mi tarjeta de contacto virtual?", answer),
  ...
  return (
    <TableContainer component={Paper}>
      <Table aria-label="collapsible table">
        <TableHead>
          <TableRow>
            <TableCell component="h2" style={{ fontSize: "20px", fontWeight: "bold", textAlign: "center" }}>Preguntas Frecuentes</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row, index) => (
            <Row key={index} row={row} setOpen={setOpen} open={open} index={index}/>
          ))}
        </TableBody>
      </Table>
    </TableContainer>
